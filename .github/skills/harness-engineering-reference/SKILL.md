...
4. **Read-only agents** (`planejador`, `revisor`, `criador-conteudo`) must NOT declare `edit` or `execute`.
...
|-----------|---------|-------------|------------|-----------------|
| **Coordinator** | Orchestrates multi-agent workflows, manages HITL gates | Full toolkit + `agent` | User-facing + subagent-invocable | `orquestrador` |
| **Worker** | Executes a pipeline phase — structured input → structured output | Domain-specific | Subagent-invocable | `planejador`, `implementador`, `revisor` |
| **Specialist** | Deep domain expertise, operates independently | Domain-specific tools | User-invocable or on-demand | `criador-conteudo`, `performance-auditor`, `refactor-css` |
| **Auditor** | Read-only analysis, produces reports — NEVER modifies | read, search only | On-demand or post-cycle | `revisor` (review mode) |
...
See `engenheiro-de-harness.agent.md` for complete SQL templates and Chronicle analysis procedures.
